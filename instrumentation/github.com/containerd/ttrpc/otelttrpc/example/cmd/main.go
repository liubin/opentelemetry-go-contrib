...
func dummy_operation(ctx context.Context) {
...
func (s *exampleServer) Method1(ctx context.Context, r *example.Method1Request) (*example.Method1Response, error) {
	...
	// and then call dummy_operation, this will generate another span that having the same level with server_client
	dummy_operation(ctx)
...
func server_client(ctx context.Context) error {
	// STEP 2: in Method1, we will first call server_client, this will generate span named server_client
	tracer := otel.Tracer("billing_app")
